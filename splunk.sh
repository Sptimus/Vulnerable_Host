...
sudo docker run -d --restart unless-stopped --mount source=logs-suricata,target=/mnt/suricata --mount source=logs-webgoat,target=/mnt/webgoat --mount source=logs-juice-shop,target=/mnt/juice-shop --mount source=logs-mutillidae_2,target=/mnt/mutillidae_2 --mount source=logs-dvwa,target=/mnt/dvwa --mount source=logs-nginx,target=/mnt/nginx --mount source=logs-hackazon,target=/mnt/hackazon -p 8000:8000 -p 8088:8088 -e "SPLUNK_START_ARGS=--accept-license" -e "SPLUNK_PASSWORD=Admin1234" --name splunk splunk/splunk:latest
